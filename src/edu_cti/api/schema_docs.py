...
PIPELINE_LAYERS: List[PipelineLayerDoc] = [
    ...
    PipelineLayerDoc(
        ...
        description="Writes all scalar values to incident_enrichments_flat for fast SQL analytics. "
                    "JSON arrays stored as serialised TEXT. New columns added via ALTER TABLE migration.",
    ),
]
...
FLAT_COLUMNS: List[ColumnDoc] = [
    ...
    ColumnDoc(name="initial_access_vector", type="TEXT",
              description="Simplified initial access: phishing, credential_theft, vulnerability_exploit, etc.",
              extraction_field="initial_access_vector", populated_by="llm",
    ...
    ColumnDoc(name="detection_source", type="TEXT", populated_by="llm",
              extraction_field="detection_source",
              description="[NEW] How the incident was discovered: internal_security / external_report / threat_actor_disclosure / ransom_note."),
...
ANALYTICS_ENDPOINTS: List[dict] = [
    ...
    {"path": "/api/analytics/attack-vectors", "tag": "Analytics",
     "fields_used": ["attack_vector", "initial_access_vector"],
...
SCHEMA_DOC = SchemaResponse(
    ...
    tables=[
        ...
        TableDoc(
            ...
            description="Denormalised flat table — one row per enriched incident. "
                        ...
                        "New columns are added via ALTER TABLE migration at startup (no data loss).",
        ...
        TableDoc(
            ...
            description="Full JSON backup of each CTIEnrichmentResult. Preserves the complete structured "
                        "LLM output even when flat table columns expand. Used for reprocessing/backfill.",
            ...
            columns=[
                ...
                ColumnDoc(name="enrichment_data", type="TEXT (JSON)",
                          description="Full CTIEnrichmentResult serialised as JSON.",
                          populated_by="llm"),
                ColumnDoc(name="enrichment_version", type="TEXT",
                          description="Pipeline version that created this record (e.g. '2.0').",
                          populated_by="pipeline"),
                ColumnDoc(name="enrichment_confidence", type="REAL",
                          description="Overall confidence score.", populated_by="llm"),
            ],
        ),
        TableDoc(
            name="articles",
    ...
    notes=[
        ...
        "New columns tagged [NEW] are auto-migrated via ALTER TABLE in init_incident_enrichments_table() — safe to deploy.",
