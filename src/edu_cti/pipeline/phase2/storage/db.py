...
def _flatten_enrichment_for_db(enrichment: CTIEnrichmentResult) -> Dict[str, Any]:
    """
    ...
    flat = {
        ...
        'institution_name': enrichment.education_relevance.institution_identified if enrichment.education_relevance else None,
        'institution_type': None,  # Not in CTIEnrichmentResult, comes from incident
        'country': None,  # Not in CTIEnrichmentResult, comes from incident
        'region': None,  # Not in CTIEnrichmentResult, comes from incident
        'city': None,  # Not in CTIEnrichmentResult, comes from incident
        'attack_category': None,
        'attack_vector': enrichment.attack_dynamics.attack_vector if enrichment.attack_dynamics else None,
        'initial_access_vector': None,
        'initial_access_description': enrichment.initial_access_description,
        'ransomware_family': enrichment.attack_dynamics.ransomware_family if enrichment.attack_dynamics else None,
        'threat_actor_name': None,
        'threat_actor_claim_url': None,
        'was_ransom_demanded': enrichment.attack_dynamics.ransom_demanded if enrichment.attack_dynamics else None,
        'ransom_amount': None,
        'ransom_currency': None,
        'ransom_paid': enrichment.attack_dynamics.ransom_paid if enrichment.attack_dynamics else None,
        'ransom_paid_amount': None,
        'data_breached': None,
        'data_exfiltrated': enrichment.attack_dynamics.data_exfiltration if enrichment.attack_dynamics else (enrichment.data_impact.get("data_exfiltrated") if enrichment.data_impact else None),
        'records_affected_exact': enrichment.data_impact.get("records_affected_exact") if enrichment.data_impact else None,
        'records_affected_min': enrichment.data_impact.get("records_affected_min") if enrichment.data_impact else None,
        'records_affected_max': enrichment.data_impact.get("records_affected_max") if enrichment.data_impact else None,
        'pii_records_leaked': None,
        'systems_affected_codes': json.dumps(enrichment.system_impact.get("systems_affected")) if enrichment.system_impact and enrichment.system_impact.get("systems_affected") else None,
        'critical_systems_affected': enrichment.system_impact.get("critical_systems_affected") if enrichment.system_impact else None,
        'network_compromised': enrichment.system_impact.get("network_compromised") if enrichment.system_impact else None,
        'email_system_affected': enrichment.system_impact.get("email_system_affected") if enrichment.system_impact else None,
        'student_portal_affected': enrichment.system_impact.get("student_portal_affected") if enrichment.system_impact else None,
        'research_systems_affected': enrichment.system_impact.get("research_systems_affected") if enrichment.system_impact else None,
        'hospital_systems_affected': enrichment.system_impact.get("hospital_systems_affected") if enrichment.system_impact else None,
        'cloud_services_affected': enrichment.system_impact.get("cloud_services_affected") if enrichment.system_impact else None,
        'third_party_vendor_impact': enrichment.system_impact.get("third_party_vendor_impact") if enrichment.system_impact else None,
        'vendor_name': enrichment.system_impact.get("vendor_name") if enrichment.system_impact else None,
        'teaching_impacted': None,
        'teaching_disrupted': enrichment.operational_impact_metrics.get("teaching_disrupted") if enrichment.operational_impact_metrics else None,
        'research_impacted': None,
        'research_disrupted': enrichment.operational_impact_metrics.get("research_disrupted") if enrichment.operational_impact_metrics else None,
        'admissions_disrupted': enrichment.operational_impact_metrics.get("admissions_disrupted") if enrichment.operational_impact_metrics else None,
        'enrollment_disrupted': enrichment.operational_impact_metrics.get("enrollment_disrupted") if enrichment.operational_impact_metrics else None,
        'payroll_disrupted': enrichment.operational_impact_metrics.get("payroll_disrupted") if enrichment.operational_impact_metrics else None,
        'classes_cancelled': enrichment.operational_impact_metrics.get("classes_cancelled") if enrichment.operational_impact_metrics else None,
        'exams_postponed': enrichment.operational_impact_metrics.get("exams_postponed") if enrichment.operational_impact_metrics else None,
        'downtime_days': enrichment.operational_impact_metrics.get("downtime_days") if enrichment.operational_impact_metrics else None,
        'outage_duration_hours': None,
        'students_affected': None,
        'staff_affected': None,
        'faculty_affected': enrichment.user_impact.get("faculty_affected") if enrichment.user_impact else None,
        'users_affected_exact': enrichment.user_impact.get("users_affected_exact") if enrichment.user_impact else None,
        'users_affected_min': enrichment.user_impact.get("users_affected_min") if enrichment.user_impact else None,
        'users_affected_max': enrichment.user_impact.get("users_affected_max") if enrichment.user_impact else None,
        'recovery_costs_min': enrichment.financial_impact.get("recovery_costs_min") if enrichment.financial_impact else None,
        'recovery_costs_max': enrichment.financial_impact.get("recovery_costs_max") if enrichment.financial_impact else None,
        'legal_costs': enrichment.financial_impact.get("legal_costs") if enrichment.financial_impact else None,
        'notification_costs': enrichment.financial_impact.get("notification_costs") if enrichment.financial_impact else None,
        'insurance_claim': enrichment.financial_impact.get("insurance_claim") if enrichment.financial_impact else None,
        'insurance_claim_amount': enrichment.financial_impact.get("insurance_claim_amount") if enrichment.financial_impact else None,
        'business_impact': enrichment.attack_dynamics.business_impact if enrichment.attack_dynamics else None,
        'gdpr_breach': enrichment.regulatory_impact.get("gdpr_breach") if enrichment.regulatory_impact else None,
        'hipaa_breach': enrichment.regulatory_impact.get("hipaa_breach") if enrichment.regulatory_impact else None,
        'ferpa_breach': enrichment.regulatory_impact.get("ferc_breach") if enrichment.regulatory_impact else None,
        'breach_notification_required': enrichment.regulatory_impact.get("breach_notification_required") if enrichment.regulatory_impact else None,
        'notifications_sent': enrichment.regulatory_impact.get("notifications_sent") if enrichment.regulatory_impact else None,
        'fine_imposed': enrichment.regulatory_impact.get("fine_imposed") if enrichment.regulatory_impact else None,
        'fine_amount': enrichment.regulatory_impact.get("fine_amount") if enrichment.regulatory_impact else None,
        'lawsuits_filed': enrichment.regulatory_impact.get("lawsuits_filed") if enrichment.regulatory_impact else None,
        'class_action': enrichment.regulatory_impact.get("class_action") if enrichment.regulatory_impact else None,
        'recovery_timeframe_days': enrichment.attack_dynamics.recovery_timeframe_days if enrichment.attack_dynamics else None,
        'recovery_started_date': enrichment.recovery_metrics.get("recovery_started_date") if enrichment.recovery_metrics else None,
        'recovery_completed_date': enrichment.recovery_metrics.get("recovery_completed_date") if enrichment.recovery_metrics else None,
        'from_backup': enrichment.recovery_metrics.get("from_backup") if enrichment.recovery_metrics else None,
        'mfa_implemented': enrichment.recovery_metrics.get("mfa_implemented") if enrichment.recovery_metrics else None,
        'incident_response_firm': enrichment.recovery_metrics.get("incident_response_firm") if enrichment.recovery_metrics else None,
        'forensics_firm': enrichment.recovery_metrics.get("forensics_firm") if enrichment.recovery_metrics else None,
        'public_disclosure': enrichment.transparency_metrics.get("public_disclosure") if enrichment.transparency_metrics else None,
        'public_disclosure_date': enrichment.transparency_metrics.get("public_disclosure_date") if enrichment.transparency_metrics else None,
        'disclosure_delay_days': enrichment.transparency_metrics.get("disclosure_delay_days") if enrichment.transparency_metrics else None,
        'transparency_level': enrichment.transparency_metrics.get("transparency_level") if enrichment.transparency_metrics else None,
        'timeline_json': json.dumps([e.model_dump() for e in enrichment.timeline]) if enrichment.timeline else None,
        'timeline_events_count': len(enrichment.timeline) if enrichment.timeline else 0,
        'mitre_techniques_json': json.dumps([t.model_dump() for t in enrichment.mitre_attack_techniques]) if enrichment.mitre_attack_techniques else None,
        'mitre_techniques_count': len(enrichment.mitre_attack_techniques) if enrichment.mitre_attack_techniques else 0,
        'enriched_summary': enrichment.enriched_summary,
        'extraction_notes': enrichment.extraction_notes,
        'confidence': None,  # Not in CTIEnrichmentResult
    }
...
def save_enrichment_result(
    ...
    flat_data = _flatten_enrichment_for_db(enrichment_result)
