...
class V2RuntimeService:
    ...
    def __init__(
        ...
        *,
        ...
        enrich_worker_count: Optional[int] = None,
        task_type: Optional[str] = None,
        ...
        if enrich_worker_count is None:
            # Dedicated enrich_source pool so the cheap, bursty classify_titles
            # stream can never starve LLM extraction (they shared the generic
            # pool before). 0 when pinned to a single task_type.
            enrich_worker_count = max(1, self.worker_count // 2) if task_type is None else 0
        ...
        self.enrich_worker_count = max(enrich_worker_count, 0)
    ...
    def start(self) -> dict[str, object]:
        ...
        if self.task_type is None:
            ...
            for index in range(self.enrich_worker_count):
                worker_id = f"v2-runtime:enrich:{index + 1}"
                state = _WorkerThreadState(
                    worker_id=worker_id,
                    thread=None,
        ...
        self._running = True
        logger.info(
            "Started v2 runtime: workers=%s enrich_workers=%s fetch_workers=%s resolve_workers=%s canonicalize_workers=%s task_type=%s scheduler=%s",
            self.worker_count,
            self.enrich_worker_count,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--enrich-workers",
        type=int,
        default=_env_int("ENRICH_WORKER_COUNT", "EDU_CTI_V2_ENRICH_WORKER_COUNT"),
        help="Number of dedicated enrich_source worker threads (0 = auto)",
    )
...
def main() -> None:
    ...
    runtime = V2RuntimeService(
        ...
        enrich_worker_count=args.enrich_workers,
